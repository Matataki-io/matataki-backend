...
class OrderHeaderService extends Service {
  ...
  async updateOrder(userId, tradeNo, useBalance) {
    ...
    try {
      ...
      // 处理到分，向上取整
      amount = Math.ceil(amount / 100) * 100;
      await conn.query('UPDATE order_headers SET amount = ?, use_balance = ? WHERE trade_no = ? AND status = 0;', [ total, tradeNo, useBalance ]);
  ...
  async handling(tradeNo) {
    ...
    try {
      ...
      const order = await this.service.shop.order.get(tradeNo);
      // 查询是否满足持币数量
      const isHold = this.service.post.isHoldMineTokens(order.signid, orderHeader.uid);
      if (!isHold) {
        await conn.rollback();
        return -4;
      }
