...
class WxpayService extends Service {
  ...
  async unifiedOrder(title, total, symbol) {
    ...
    const payargs = await this.app.tenpay.unifiedOrder({
      body: title,
  ...
  async refund(out_trade_no, total_fee, refund_fee) {
    ...
    const refund_order = {
      ...
      notify_url: 'https://apitest.smartsignature.io/wx/refundNotify',
    };
    ctx.logger.info('service refund refund_order', refund_order);
    // TODO 查找订单号
    const payargs = await this.app.tenpay.refund(refund_order);
    return payargs;
  }
}
