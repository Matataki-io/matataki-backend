...
class NotificationController extends Controller {
  ...
  async getEventGgroupsByUid() {
    ...
    const announcementIdSet = new Set();
  ...
  async getEventByRegion() {
    ...
    eventList.list.forEach(item => {
      ...
      { commentIdSet.add(Number(item.remark)); } else if (item.action === 'transfer' && objectType === 'article') // 打赏文章
      { minetokensLogIdSet.add(Number(item.remark)); } else if (objectType === 'collaborator') // 协作者
