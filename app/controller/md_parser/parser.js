...
function attrBoolean(val,def){
    return val ? val==='false'||val==='0' || val==='null' 
        ? false : true : def;
}
...
function holdMines(user,mines,balanceOf){
    ...
    while (α < mines.length) {
        if (balanceOf(user,mines[α].token) < mines[α].amount)
            return false;
        α++;
    }
    return true;
}
export function execute(ast,{userId,balanceOf}) {
    let α = 0, β = '';
    while (α < ast.length){
        if (ast[α].block === 'text') {
            β+=ast[α].value;
